...
my $home=$ENV{'HOME'};
my $logname="/tmp/ck_alertlog.log";
my $admindir;
my @v_cputtime;
my %Hproctimes;
my $pid;
...
my $dberrstr = 'ORA-';
my $lsnrerr='ORA-12541: TNS:no listener';
...
my @mailtolist = ('dong.faying@dol.gov','black.michael@dol.gov','Amerman.Shane@dol.gov','Lay.Coy@dol.gov', 'smith.bradford@dol.gov');
#my @mailtolist = ('dong.faying@dol.gov','black.michael@dol.gov');
...
#		if (($lines[$element] =~ /^$dberrstr/)||($lines[$element] =~ /^$altdberrstr/)) {
		if ((($lines[$element] =~ /^$dberrstr/)&&!($lines[$element]=~/$lsnrerr/))||($lines[$element] =~ /^$altdberrstr/)) {
			$mailit=1;
			$pageit=1;
