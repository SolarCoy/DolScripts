...
function get_arch_dest {
   ...
   STR_ARCH_DIR=$(print ${RET_VAL}|grep ${SID}|awk -F= '{print $2}')
...
#  tmp
function mail_file_tmp {
  if [[ $SEND_MAIL = $TRUE ]]
  then 
     typeset SBJCT=$1 
     typeset FNAME=$2 
     
     set -A MAIL_ARRAY ${MAIL_LIST1} 
     mailx -s "$SBJCT" ${MAIL_ARRAY[*]} <$FNAME
     if (( $? != ${STAT_OK} ))
     then
           print "Warning: Mailing file failed."
  ...
  if (( $? != ${STAT_OK} ))
     ...
     then
       ...
       then
  	  ...
  	  scp -p ${SRC_ARCH_DIR}/${PRE}${SEQ}${EXT}.gz \
          ...
          if (( $? != ${STAT_OK} ))
          then
              print "  Remote copy ${PRE}${SEQ}${EXT} failed."
     	      return ${ST_FAIL}
          fi
   ...
   do
	...
	${GZIP} ${SRC_ARCH_DIR}/${FNAME} 2>/dev/null
        ...
        if (( $? != ${STAT_OK} ))
        then
              print "  Remote copy ${PRE}${FSEQ}${EXT} failed."
             return ${ST_FAIL}
        fi
                let CNT=$CNT+1
      fi
    done
   # print "start $START_SEQ" 
   # print "end  $END_SEQ" 
    return ${STAT_OK}
}
#***********************************************************************
