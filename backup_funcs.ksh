...
function  do_hot_bkup_sid { 
  ...
  print "  Compress and backup archive logs to remote site ${SID_RMT_TOP_DIR}/arch." \
	|tee -a ${LOGFILE}
  do_arch_logs $SID ${SID_RMT_TOP_DIR}
  if (( $? != ${STAT_OK} ))
  then
  ...
  print " "
  print "  Copy pfile(spfile) and pwd file to ${SID_RMT_TOP_DIR}/misc." \
	|tee -a ${LOGFILE}
  do_pwd_spfile_bkup
  if (( $? != ${STAT_OK} ))
  then
...
function  do_misc_bkup_sid { 
  ...
  print " "
#  print "  Copy pfile(spfile) and pwd file to ${SID_RMT_TOP_DIR}/misc." \
  print "  Copy pfile(spfile) and pwd file to ${SID_LOC_TOP_DIR}/misc." \
	|tee -a ${LOGFILE}
  do_pwd_spfile_bkup
  if (( $? != ${STAT_OK} ))
  then
...
function do_tablespace_bkup {
   ...
   PREV_TABLESPACE="NON_EXIST_TBLSPS"
   eval $SED|while read TABLESPACE DATA_FILE
   do
   ...
   then  
        ...
        if (( $? != ${STAT_OK} ))
        then
               print "Remote copy ${DATA_FILE}.Z failed."
               return ${ST_FAIL}
        fi
   else
        ${ZIP}< ${DATA_FILE}>${DATA_FILE}.Z 2>/dev/null
...
typeset SQL="alter system set user_dump_dest='${SID_MISC_DIR}';
       ...
       alter system set user_dump_dest='${ADMIN_DIR}/${ORACLE_SID}/udump';
...
#	${REMOTE_ACCT}:${SID_RMT_TOP_DIR}/misc
  ...
  if (( $? != ${STAT_OK} ))
  then
#         print "  Remote copy spfile${ORACLE_SID}.ora failed." |tee -a $LOGILFE
...
function do_arch_logs  {
  ...
  # Get achive log destination
   ...
   print "  Compress and remote copy archive logs of $SID to "
   print "${SID_RMT_TOP_DIR}."

   compress_copy_arch_logs $SID 
   if (( $? != ${STAT_OK} ))
   then
        print "  Compress and Remote copy archive logs failed."|tee -a $LOG
        return ${ST_FAIL}
   fi
  print "*******************************************************"|tee -a $LOG
...
function do_export_sid {
  ...
  rm -f ${ADMIN_DIR}/${SID}/exp/par_*.lst >/dev/null 2>&1
  >${PARFILE}
  chmod 600 ${PARFILE}
...
function rm_loc_old_dir {
   ...
   typeset -i CNT=0
   typeset DIR0=""
   find ${ADMIN_DIR}/$SID/backup/* -type d -mtime \
      +${DAYS_KEEP_BACKUPS}|grep -v exp|while read FILE
   do
      typeset    DIR1=$(print $FILE|awk -F/ '{print $9 }')
      if [[ $DIR1 != $DIR0 ]];
        then
           DIR_ARR[$CNT]=$DIR1
           CNT=$CNT+1
   ...
   while [[ ${CNT} -lt ${#DIR_ARR[*]} ]]
   do
       print "rm -r ${ADMIN_DIR}/${SID}/backup/${DIR_ARR[$CNT]} "
       rm -r ${ADMIN_DIR}/${SID}/backup/${DIR_ARR[$CNT]} 
       CNT=$CNT+1
       sleep 10
   done
}
